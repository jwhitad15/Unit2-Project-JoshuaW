...
//       try {
//         const url = `${ApiHelper.baseUrl}/${type}`;
...
//       try {
//         const url = `${ApiHelper.baseUrl}/questions/add`;
...
//       try {
//         const url = `${ApiHelper.baseUrl}/scriptures/add`;
...
//         const url = id.trim()
//           ? `${ApiHelper.baseUrl}/${id.trim()}` // Fetch specific record by ID
//           : `${ApiHelper.baseUrl}/${type}`; // Fetch all records of the selected type
...
//       try {
//         const url = `${ApiHelper.baseUrl}/${type}/update/${id.trim()}`; // Construct the API URL
...
//         const url = `${ApiHelper.baseUrl}/${type}/${command}/${deleteID}` // Fetch specific record by ID
...
const AdminQuery = ({ setFetchedData, setSelectedType, userData, questionData, scriptureData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (command === "add") {
      try {
        const dataToAdd = type === "users" ? userData : type === "questions" ? questionData : scriptureData;
        const url = `${ApiHelper.baseUrl}/${baseEndpoint}/add`;
        const response = await fetch(url, {
          method: "POST",
        ...
        // Fetch updated full list
        const listResponse = await fetch(`${ApiHelper.baseUrl}/${baseEndpoint}`);
        const listData = await listResponse.json();
        setFetchedData(listData);
        setErrorMessage("");
      } catch (error) {
        setErrorMessage("Failed to add data. Please try again.");
      }
    ...
    if (command === "view") {
      try {
        const url = id.trim()
          ? `${ApiHelper.baseUrl}/${baseEndpoint}/${id.trim()}`
          : `${ApiHelper.baseUrl}/${baseEndpoint}`;
        const response = await fetch(url, {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        });
        ...
        const data = await response.json();
        setFetchedData(id.trim() ? [data] : data);
        setErrorMessage("");
      } catch (error) {
        setErrorMessage("Failed to fetch data. Please try again.");
      }
    ...
    if (command === "update") {
      ...
      try {
        const dataToUpdate = type === "users" ? userData : type === "questions" ? questionData : scriptureData;
        const url = `${ApiHelper.baseUrl}/${baseEndpoint}/update/${id.trim()}`;
        const response = await fetch(url, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
    ...
    if (command === "delete") {
      try {
        if (!id.trim()) {
          setErrorMessage("Please provide an ID to delete the record.");
          return;
        }
        const url = `${ApiHelper.baseUrl}/${baseEndpoint}/delete/${id.trim()}`;
        const response = await fetch(url, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
        });

        if (!response.ok && response.status !== 204) throw new Error("Failed to delete data.");
